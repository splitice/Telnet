...
class TelnetClient {
	...
	function initial_options() {
		if (! $this->mode ['telnet_bugs']) {
			/*
			 * Some implementations only keep a single SGA state, though rfc858 says it must be suppressed in both directions independently.
			 */
		...
		if (! $this->mode ['telnet_bugs']) {
			/*
			 * Working around a buggy telnet that strips the BINARY (ie. ASCII 0) out of the datastream by not requesting it.
			 */
	 ...
	 *        	
	 * @return integer boolean of bytes read or false on error
	 */
					...
					case TEL_DONT :
						if (! feof ( $s ) && ($opt = fgetc ( $s )) === false) {
							...
							if ($info ['eof'])
								...
								$this->debug ( "Error reading TELNET option " . " char for {$TELCMDS[$c]} command" );
								continue;
							} else {
								throw new \Exception ( "Error reading TELNET option " . " char for {$TELCMDS[$c]} command" );
							}
						}
